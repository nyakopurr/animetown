...
local flux = (function()
    ...
    return CompileString("return function(p) " .. str:gsub("%$e", expr) .. " end", "TestFunction")()
    end
...
surface.CreateFont( "AT_Leveling", { font = "Rubik Medium", size = 15, weight = 500, extended = true } )

local baseMat = Material( "repix/animetown/hud/leveling/base.png" )
local xpMat = Material( "repix/animetown/hud/leveling/xp.png" )

local lastXP = 0

hook.Add( "Think", "repix_AnimeTown_Leveling", function()
    local xp = LocalPlayer():XP()
    if ( lastXP ~= xp ) then
        hook.Call( "OnXPChange", GAMEMODE )
        lastXP = xp
    end
end )

local Animation = { alpha = 0, xp = 0 }
hook.Add( "OnXPChange", "repix_AnimeTown_Leveling", function()
    if ( flux && flux.to ) then
        flux.to( Animation, 0.5, { xp = LocalPlayer():XP(), alpha = 1 } )
        :after( Animation, 0.5, { alpha = 0 } )
        :delay( 4 )
    end
end )

hook.Add( "HUDPaint", "repix_AnimeTown_DrawXP", function()
    if ( Animation.alpha > 0 ) then
        surface.SetDrawColor( 255, 255, 255, 255 * Animation.alpha )
        surface.SetMaterial( baseMat )
        surface.DrawTexturedRect( 28, 45, baseMat:Width(), baseMat:Height() )

        surface.SetMaterial( xpMat )
        render.SetScissorRect( 32, 45, 32 + ( xpMat:Width() * ( Animation.xp / ( 500 + ( math.floor( LocalPlayer():Level() * 100 ) ) ) ) ), ScrH(), true )
            surface.DrawTexturedRect( 32, 47, xpMat:Width(), xpMat:Height() )
        render.SetScissorRect( 0, 0, 0, 0, false )

        draw.SimpleText( math.floor( Animation.xp ) .. "xp | уровень " .. LocalPlayer():Level(), "AT_Leveling", 160, 64, Color( 0, 0, 0, 100 * Animation.alpha ), 1 )
        draw.SimpleText( math.floor( Animation.xp ) .. "xp | уровень " .. LocalPlayer():Level(), "AT_Leveling", 160, 63, Color( 255, 255, 255, 255 * Animation.alpha ), 1 )
    end
